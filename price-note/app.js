...
function renderList(){count.textContent=`${products.length}商品`;if(!products.length){list.innerHTML='<div class="product-empty">商品はまだありません<br>上のフォームから保存してください</div>';return}list.innerHTML=products.map(item=>`<div class="product-row"><div><h3>${esc(item.name)}</h3><p>${item.unit?esc(item.unit)+' ':''}¥${money(item.price)} ${item.tax?`／ ${esc(item.tax)}`:''}</p></div><div class="product-row-actions"><button type="button" data-edit="${item.id}">編集</button><button type="button" class="danger" data-delete="${item.id}">削除</button></div></div>`).join('')}
list.addEventListener('click',e=>{const edit=e.target.closest('[data-edit]');if(edit){const item=products.find(x=>x.id===edit.dataset.edit);if(!item)return;editingId.value=item.id;nameInput.value=item.name;priceInput.value=item.price;unitInput.value=item.unit;taxInput.value=item.tax;sizeInput.value=item.size;allergyInput.value=item.allergens||'';noteInput.value=item.note||'';bgInput.value=item.bg||'#101c32';fgInput.value=item.fg||'#ffffff';titleSizeInput.value=item.titleSize||38;document.querySelector('#save-card').textContent='更新する';document.querySelectorAll('[data-allergen]').forEach(b=>b.classList.toggle('active',(item.allergens||'').split(/[、,]/).map(s=>s.trim()).includes(b.dataset.allergen)));renderPreview();window.scrollTo({top:0,behavior:'smooth'});return}const del=e.target.closest('[data-delete]');if(del){const item=products.find(x=>x.id===del.dataset.delete);if(item&&confirm(`${item.name} を削除しますか？`)){products=products.filter(x=>x.id!==item.id);saveProducts();renderList();showToast('削除しました')}}});
